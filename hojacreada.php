...
?> </center>
	...
	if(!empty($_POST))
	{
		$valor = $_POST['campo'];
		if(!empty($valor)){
			$where = "WHERE Numero_documento LIKE '%$valor'";
		}
	}
	$sql = "SELECT id, Nombres, Tipo_documento, Numero_documento, Telefono, Direccion, Edad, Tipo_afiliacion, Origen, Zona,  Cita_asignada, Fecha_cita, Cumplio_cita, Recibio_medicamento FROM paciente $where";
...
<div class="container col-sm-12">
      ...
      <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST">
					...
					<b> Buscar paciente: </b><input type="text" id="campo" name="campo" />
					<input type="submit" id="enviar" name="enviar" value="Buscar" class="btn btn-success" />
				</form>
        <ul class="navbar-nav ml-auto mr-0 mr-md-3 my-2 my-md-0">
        <div class="exportar">
        <a href="conexport.php" class="btn btn-warning">Exportar</a>
        </div>
        </ul>
        </div>
			<br>
            ...
            while($row = $resultado->fetch_array(MYSQLI_ASSOC)) { ?>
							<tr font size="1" >
								...
								<td ><?php echo $row['Cita_asignada']; ?></td>
                <td>
                <?php
              
              $fecha_actual = strtotime(date("d-m-Y H:i:00",time()));
              $fecha_cita = strtotime($row['Fecha_cita']);
              $fecha= strtotime($row['Fecha_cita']."- 5 days");
              if($fecha_actual >  $fecha && $fecha_actual < $fecha_cita){
                $el_correo_user="kajero1068390997@gmail.com";
                //mail($el_correo_user, 'Tu posts esta vencido', 'Tu post esta vencido, visita www.dominio.com para renovarlo');
                mail($el_correo_user, 'Tu posts esta vencido', 'Tu post esta vencido, visita www.dominio.com para renovarlo');
                echo '<span style="color: orange">' .$row['Fecha_cita']. '</span>';
              }
              elseif($fecha_actual > $fecha){
                       echo '<span style="color:red">' .$row['Fecha_cita']. '</span>';
                 }else{
                  echo '<span style="color:green">' .$row['Fecha_cita']. '</a>';
               }
              ?>
								...
								<td><a  class= "btn btn-outline-success" href="actualizar.php?id=<?php echo $row['id']; ?>"><span ></span>Actualizar</a></td>
								<td><a  class= "btn btn-outline-danger btn-ok" href="#" data-href="eliminar.php?id=<?php echo $row['id']; ?>" data-toggle="modal" data-target="#confirm-delete"><span class="glyphicon glyphicon-trash"></span>Eliminar</a></td>
                </tr>
						<?php } ?>
					</tbody>
				</table>
